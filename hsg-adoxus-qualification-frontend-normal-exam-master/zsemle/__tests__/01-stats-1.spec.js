...
describe("Stats table", () => {
  ...
  gradeIt({
    ...
    test: async function () {
      ...
      const expectedCountSum = sumProductCount();
      const expectedPriceSum = sumProductPrice();
      ...
      expect(
        Number(summaryCells[2].textContent.match(/[0-9]+/g).join("")),
        `The calculated sum price is wrong`
      ).toBe(expectedPriceSum);
    },
  });
});
